# Copyright Nastase Robert-Ionut 312CA
...
async def join(ctx):
    ...
    if ctx.voice_client == None:
        ...
        await ctx.send('I already joined the voice channel')


@bot.command(brief='List all the songs')
async def list(ctx):
    cnt = 1
    songs = os.listdir('/home/student/Desktop/Bot_IDST/songs')
    for song_name in songs:
        await ctx.send(str(cnt) + '. ' + song_name + '\n')
        cnt += 1

@bot.command(brief='Play a song from a local directory')
async def play(ctx, index : int):
    ...
    if ctx.voice_client is None:
        ...
        voice.play(FFmpegPCMAudio(songs[index - 1]))
        await ctx.send('Playing: ' + songs[index - 1])
    else:
        voice = await voice_channel.connect()
        voice.play(FFmpegPCMAudio(songs[index - 1]))
        await ctx.send('Currently changing to: ' + songs[index - 1])

@bot.command(brief='Disconnect from a channel')
async def scram(ctx):
    await ctx.voice_client.disconnect()
    await ctx.send('I just left the channel.')

@bot.command(brief='Disconnect from a channel if there are no members left')
async def on_voice_state_update():
    member_count = len(voice_channel.members)
    if member_count == 1:
        await asyncio.sleep(30)
    voice.loop.create_task(on_voice_state_update)
...
"""
    Discord Music Bot
    Copyright (C) 2021  Robert Nastase
